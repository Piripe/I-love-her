...
export async function POST(req: Request) {
    ...
    const data = (await req.json()) as {id:number; priority: number; layout: LayoutDefinition, title:string };

    const success = await updateFallback(data.id,data.priority, data.layout, data.title);
